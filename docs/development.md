...
修改并分发安装包时请递增版本号，避免 pnpm 复用同名、同版本的旧包缓存。0.3.4 让活跃会话里输入区外圈的底板透明、露出背景，卡片自身的表面保留；播放条仍高于输入区、低于原生触发菜单和弹窗，与输入框可能重叠时仅上移播放条。0.3.3 去掉了 0.3.2 的 52px 留白，恢复原生输入区位置。窗口、输入框尺寸和会话节点变化触发重新定位，卸载时清理观察器和监听器，没有常驻轮询。
