...
修改并分发安装包时请递增版本号，避免 pnpm 复用同名、同版本的旧包缓存。0.3.5 让播放条按真实几何自适应：输入卡占住右下角时隐藏「尼可 · x/14」计数并收窄回角；收窄横条也放不下（窄窗/竖屏）时改竖排、仅悬在输入区上方 8px，保留暂停键，仍不遮发送按钮。0.3.4 让活跃会话里输入区外圈的底板透明、露出背景，卡片自身的表面保留。0.3.3 去掉了 0.3.2 的 52px 留白，恢复原生输入区位置。播放条悬浮时低于原生触发菜单和弹窗，与输入框可能重叠时只调整自身，不动宿主布局。窗口、输入框尺寸和会话节点变化触发重新定位，卸载时清理观察器和监听器，没有常驻轮询。
...
单元测试有 35 项，安装后浏览器测试 18 项，卸载后测试 1 项。覆盖素材裁切、双向循环、五档计时、暂停、后台标签页、减少动态效果、偏好恢复、断网、浅深色和窄屏。
