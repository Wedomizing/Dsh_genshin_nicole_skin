...
test('official uninstall restores the native DSH conversation background', async ({ page }) => {
  ...
  await expect(page.locator(CONVERSATION_ROOT)).toBeVisible();
  ...
  expect(await page.locator(CONVERSATION_ROOT).evaluate((el) => getComputedStyle(el).backgroundColor)).not.toBe('rgba(0, 0, 0, 0)');
